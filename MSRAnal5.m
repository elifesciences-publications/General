...
dataLoading = questdlg('How would you like to load your data? ','Selecting data','Workspace','Other','Workspace');
switch dataLoading
    case 'Workspace'
        ...
        nLines=1;
        def = {'channels','sweepTime'};
        answer=inputdlg(prompt,dlgTitle,nLines,def);
        dataVar=eval(answer{1});
        timeVar=eval(answer{2});
        si=mean(diff(timeVar));

    case 'Other'
        %% Select and load the file into the matlab workspace and save it in the variable name 'data'
