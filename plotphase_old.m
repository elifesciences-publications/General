...
if avgCheck ==1
        Wxy_nonzero_lin = Wxy_avg; Wxy_nonzero_lin(Wxy_nonzero_lin==0)=[]; % This removes all zero elements from the matrix and vectorizes it.
...
rtl = [max(phase_dist_weight.(fStr).(chStr))/2 max(phase_dist_weight.(fStr).(chStr))];
...
for fileNum = 1:size(sigMat,2) % File Number Loop # 1
    ...
    for chNum =1:size(sigMat,3)-1 % Channel Number Loop # 1
        ...
        cellNum = cellNum+1;
        % XW power-weighted rose diagram
       figure('Name', ['Rose Diagram, File ' num2str(fileNum) ', Channels '...
            num2str(ch(chNum)) ' vs ' num2str(ch(chNum+1))],'color','w')
        handle = mmpolar(theta.(fStr).(chStr),phase_dist_weight.(fStr).(chStr),...
            'b-','border',border_toggle,'ttickdelta',ttdelta,...
            'rtickvalue',rtl,'border','on','tticklabel',ttl);
        set(gcf,'color','w'), set(handle,'linewidth',2)
        hold on
      ...
      % Weighted polar plot  
%       polar(angle(Wxy_nonzero_lin(:)),abs(Wxy_nonzero_lin(:))/max(abs(Wxy_nonzero_lin)),'-')
        % Unweighted rose diagram
         handle = mmpolar(theta.(fStr).(chStr),phase_dist.(fStr).(chStr),...
            'color',[0 0.5 0],'linestyle',':','border',border_toggle,'ttickdelta',ttdelta,...
            'rtickvalue',rtl,'border','on','tticklabel',ttl);
        set(gcf,'color','w'), set(handle,'linewidth',2,'markersize',15)
         
         mp = meanPhases.(fStr).(chStr)*pi/180;
        pol = polar(mp*ones(1,2),[0 1]);
        set(pol,'color','r','linewidth',2,'linestyle','--','linewidth',2)
        legend('Weighted', 'Unweighted','Mean Phase')
        hold off
