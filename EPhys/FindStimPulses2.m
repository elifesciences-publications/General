...
dData=diff(signals); % 2nd derivative gives the highest value for stimulus artifact onset
slopeThresh= mean(dData) + slopeThresh*std(dData);
transients=find(dData >= slopeThresh);
else 
 transients  = [];
end
